...
def voxelize_all_with_calculated_resolution(dims, voxels_per_unit, dir_, file_prefix, file_ext, **kwargs):
    ...
    def voxelize(idx):
        voxelize_file(filename(idx), long_axis_resolution=int(max(dims[base_name(idx)]['dim']) * voxels_per_unit), **kwargs)
...
def voxelize_all(dir_, file_prefix, file_ext, **kwargs):
    """ Uses Stanford Haptics Lab's 'Voxelizer' to voxelize all files in the given directory with the given prefix and extension
